...
// NewServer creates a new api jili server but does not configure it
func NewServer(api *operations.JiliAPI) *Server {
	s := new(Server)
	s.api = api
	return s
}
...
// Server for the jili API
type Server struct {
	EnabledListeners []string `long:"scheme" description:"the listeners to enable, this can be repeated and defaults to the schemes in the swagger spec"`

	SocketPath    flags.Filename `long:"socket-path" description:"the unix socket to listen on" default:"/var/run/jili.sock"`
...
func (s *Server) SetAPI(api *operations.JiliAPI) {
	if api == nil {
		s.api = nil
		s.handler = nil
		return
...
func (s *Server) Serve() (err error) {
	...
	if s.hasScheme(schemeUnix) {
		...
		wg.Add(1)
		s.Logf("Serving jili at unix://%s", s.SocketPath)
	...
	if s.hasScheme(schemeHTTP) {
		...
		wg.Add(1)
		s.Logf("Serving jili at http://%s", s.httpServerL.Addr())
		go func(l net.Listener) {
			defer wg.Done()
	...
	if s.hasScheme(schemeHTTPS) {
		...
		wg.Add(1)
		s.Logf("Serving jili at https://%s", s.httpsServerL.Addr())
		go func(l net.Listener) {
			defer wg.Done()
